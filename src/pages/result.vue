...
import { getConfig, getCopyBundles, getNeutral, getSignContent, getSignProfile, type Locale } from '~/content'
...
const profile = computed(() => (state.value ? getSignProfile(state.value.sign) : undefined))

const mut = computed(() => {
  if (!state.value || !profile.value) return null
  const config = getConfig()
  const eff = effectiveBaseline(profile.value.baseline, getNeutral(), state.value.ageBand, config)
...
const ctx = computed(() => {
  ...
  return {
    ...
    signName: getSignContent(locale.value as Locale, state.value.sign)?.name ?? state.value.sign,
...
const driftLabel = computed(() => {
  ...
  return getCopyBundles()[locale.value as Locale].mutationLabel[key]
})

const scores = computed(() => {
  if (!state.value) return []
  const c = getCopyBundles()[locale.value as Locale]
  const en = getCopyBundles().en
